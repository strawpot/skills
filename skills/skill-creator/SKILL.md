...
- Create a few test prompts and run an agent with the skill on them
...
The skill creator is liable to be used by people across a wide range of familiarity with coding jargon. There's a trend now where the power of AI coding tools is inspiring plumbers to open up their terminals, parents and grandparents to google "how to install npm". On the other hand, the bulk of users are probably fairly computer-literate.
...
## Before you start: does this need to be a skill?
...
- **description**: The primary triggering mechanism. Include both what the skill does AND when to use it. All "when to use" info goes here, not in the body.

  **Make descriptions pushy.** Agents tend to undertrigger skills — to not use them when they'd be useful. Combat this by explicitly listing trigger contexts.

  Bad: `"How to build a simple fast dashboard to display internal company data."`

  Good: `"How to build a simple fast dashboard to display internal company data. Make sure to use this skill whenever the user mentions dashboards, data visualization, internal metrics, or wants to display any kind of company data, even if they don't explicitly ask for a 'dashboard.'"`

  **Common mistakes:**
  - Too vague: `"Helps with documents"` — which documents? The trigger can't distinguish this from other file-handling skills.
  - Missing trigger contexts: only describing what the skill does, not when to use it.
  - Too narrow: listing only one phrasing when users say the same thing many different ways.
...
- **version**: StrawHub tracks versions automatically at publish time. Adding `version` to frontmatter causes conflicts with the registry.
- **displayName**: Auto-generated by StrawHub from the skill name. Adding `displayName` manually causes conflicts with the registry.
...
Once all runs are done, you'll grade each run against assertions, aggregate the results into a benchmark, do an analysis pass, and launch the viewer for the user. Here's the sequence:
...
Some environments (e.g., single-agent runtimes, web-based interfaces) don't support subagents. The core workflow is the same (draft → test → review → improve → repeat), but some mechanics change. Here's what to adapt:
...
**Packaging**: The `package_skill.py` script works anywhere with Python and a filesystem. The user can download the resulting `.skill` file.
...
If you're in a headless or remote environment (no browser, no display), the main things to know are:

- If you have subagents, the main workflow (spawn test cases in parallel, run baselines, grade, etc.) all works. (However, if you run into severe problems with timeouts, it's OK to run the test prompts in series rather than parallel.)
- You don't have a browser or display, so when generating the eval viewer, use `--static <output_path>` to write a standalone HTML file instead of starting a server. Then proffer a link that the user can click to open the HTML in their browser.
- Important: after running tests, you should always generate the eval viewer for the human to look at examples before revising the skill yourself and trying to make corrections, using `generate_review.py` (not writing your own custom HTML). GENERATE THE EVAL VIEWER *BEFORE* evaluating results yourself. You want to get them in front of the human ASAP!
- Feedback works differently: since there's no running server, the viewer's "Submit All Reviews" button will download `feedback.json` as a file. You can then read it from there (you may have to request access first).
- Packaging works — `package_skill.py` just needs Python and a filesystem.
- Description optimization (`run_loop.py` / `run_eval.py`) works in headless environments since it uses a CLI agent via subprocess, not a browser. Save it until you've fully finished making the skill and the user agrees it's in good shape.
